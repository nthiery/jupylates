...
def Quality_function(
    prob_of_success, avg_prob_success, exo, actual_theme, liste_activities
):
    # path
    target_directory = os.getcwd() + "/models/" + actual_theme
...
def maison(liste_activities, actual_theme):
    ...
    for i in range(len(liste_activities)):
        if df_user.loc[df_user.activity == i + 1].size == 0:
            ...
            df_temp.insert(
                i + 1,
                "TR" + str(i + 1),
                features_one_user(df_user, i + 1, actual_theme, liste_activities),
            )

    df_prob, prob_of_success = predict_success_student(
        df_temp, actual_theme, liste_activities
    )
    ...
    quality = Quality_function(
        prob_of_success, avg_prob_success, next_exercise, actual_theme, liste_activities
    )
    if quality == "Good":
